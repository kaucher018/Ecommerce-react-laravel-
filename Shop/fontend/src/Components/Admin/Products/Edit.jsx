...
const Edit = ({ placeholder }) => {
    ...
    defaultValues: async () => {
       await fetch(`${API_URL}products/${params.id}`, {
        ...
        .then((result) => {
          ...
          setproductimages(result.product.product_images);
         ...
         reset({
          ...
          compare_price: result.product.compare_price,
          Short_description: result.product.Short_description,
          quantity: result.product.quantity,
          gallary: result.product.gallary,
  ...
  const handlefile = async (e) => {
    ...
    await fetch(`${API_URL}saveproductimage`, {
      method: "POST",
      headers: {
        Accept: "Application/json",
        Authorization: `Bearer ${token()}`,
      ...
      .then((result) => {
        ...
        //  const  img= [...productimages, result.data];
          // (productimages);
...
const setdefault = async (image) => {
 await fetch(`${API_URL}setproductdefaultimage?product_id=${params.id}&image=${image}`, {
      method: "GET",
      headers: {
        "Content-type": "application/json",
        Accept: "Application/json",
...
const fetchsizes = async () => {
  ...
  const handleDeleteImage = async (id) => {
    ...
    await fetch(`${API_URL}deleteproductimage/${id}`, {
      method: "DELETE",
      headers: {
        "Content-type": "application/json",
        Accept: "Application/json",
      ...
      return (
        <div key={`image-${index}`} className="flex flex-col items-center">
          ...
          <button type="button" onClick={() => setdefault(productimage.image)} 
           ...
           <button type="button"
            onClick={() => handleDeleteImage(productimage.id)}
...
</div>text-red-500 hover:text-red-700
